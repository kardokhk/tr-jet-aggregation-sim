"""Build main Table 1 and Supplementary Tables S1 to S6 for manuscript v04 (EHJ-CVI) as markdown.

Adapted from code/14_make_tables_v02.py (v02/v03 tables), which is left unchanged so that
drafts/tables-2026-09-18-v02.md and -v03.md stay reproducible. Changes for v04:
  * new main Table 1 (four view-combination rules, plain names) built by table1_main();
  * former Tables 1 to 4 and Supplementary Table S1 become Supplementary Tables S1 to S5 with the same
    numbers; estimator labels are written as plain name plus code, "septolateral" and
    "beat-consistency window" are used throughout, and the unverified bRIGHT registry sentence is
    removed from the parameter-table footnote;
  * new Supplementary Table S6 maps plain names to the codes used in the protocol and code.
...
Inputs (read only):
  ...
  results/2026-09-18_full/analysis/E1E3_e3_base.csv                  main Table 1 (sensitivity, specificity)
  (main Table 1 also reads E1E3_e1_headline.csv and E1bE3b_e1_worstcase.csv, as for Table 2)

Table numbers in the input list above are the v02/v03 numbers: v02 Table 1 -> S1, Table 2 -> S2,
Table 3 -> S3, Table 4 -> S4, v02 Supplementary Table S1 -> S5 (function names keep the v02 numbering).

Output: drafts/tables-2026-09-19-v04.md (generated; do not hand edit).

Run from the project root (login node; no random numbers):
  /project/home/p201509/envs/duomax-sim/bin/python code/15_make_tables_v04.py
...
OUT = ROOT / "drafts" / "tables-2026-09-19-v04.md"
SCRIPT = "code/15_make_tables_v04.py"
Z = 1.96
MINUS = "−"
...
def table1(cfg, cfg2):
    ...
    add("Rules (design)", "Beat-consistency window, W", "±" + pc(v("window")),
        ...
        "design", "protocol")
    add("Rules (design)", "Largest view mean after review (A4): warning and adjudication thresholds (mm)", f"{v('t_warn'):g} and {v('t_adj'):g}",
        "warning 2, 3, 4; adjudication 4, 5, 6 (E4)", NV, "design", "protocol")
    add("Rules (design)", "Largest view mean after review (A4): detection of true overestimation; false rejection",
    ...
    title = (f"**Supplementary Table S1. Parameters of the data-generating mechanism.** No source measured the TEE colour-jet "
    ...
    foot = ("Construct match: close, vena contracta width or coaptation gap; proxy, jet area or another "
            ...
            "mechanism only, and the RR slope a direction-only aortic source, so both are counted as assumed. "
...
# ---------------------------------------------------------------- Table 2
EST = {"A1": "Anchor-view mean (A1)", "A2": "Mean across views (A2)", "A3": "Largest view mean (A3)",
       "A4": "Largest view mean after review (A4)", "A5": "Median across views (A5)", "A6": "Index beat (A6)",
       "A7": "Offset-corrected mean, benchmark (A7)"}
...
def table2():
    ...
    mx = lambda c: h[c].max()
    title = ("**Supplementary Table S2. Estimator bias and root-mean-square error against the true maximal span "
             "T1.** In the pre-specified base case the largest view mean (A3) and the largest view mean after "
             "review (A4) overestimated and the mean across views (A2) underestimated T1; across 12 combinations of anchor and long-axis underestimation, A3 and A4 "
    ...
    foot = ("Panel A, pre-specified E1 base case: K = 3 views, N = 3 beats per view, ±15% beat-consistency window, "
            ...
            "20%) and mean long-axis underestimation (8%, 20%, 40%), written anchor / long-axis. The offset-corrected "
            f"mean (A7) is excluded from panel B because it needs the true view offsets; its AP bias ranged from {f(a7.bias_min, 2)} to "
            f"{f(a7.bias_max, 2)} mm. Bias is the mean of estimate minus T1; relative bias is the mean of "
            "per-patient ratios (estimate / T1) minus 1. The offset-corrected mean (A7) divides each view mean by its "
...
def table3():
    ...
    title = ("**Supplementary Table S3. Reference quality and apparent AI performance in the base cell.** A second read or "
    ...
    foot = ("Base cell: beat CV 15%, view overestimation present, AP axis, K = 3, every read by the largest view mean after review (A4) "
            ...
            "Carlo interval; MCSE, Monte Carlo standard error; R_img, largest view mean after review (A4) computed from the images without "
...
def table4():
    ...
    title = ("**Supplementary Table S4. Precision of double-read agreement statistics and performance of sentinel tests for "
...
def tableS1():
    ...
    rows = [
        ...
        ["Same, largest view mean after review (A4) (mm)", "10.32 (8.86 to 11.99)", "9.5 [@pascalxtr2020]", "close", "calibration check",
         "Inside"],
    ...
    title = ("**Supplementary Table S5. Face validity of the base scenario against published measurements.** "
    ...
    foot = ("Simulated values from the amendment 2 face-validity analysis (code/12_face_validity.py; 2,000 "
            ...
            "bias in Supplementary Table S2. Role: a calibration check compares against a source that was used to set the "
...
# ---------------------------------------------------------------- main Table 1 (v04)
MAIN_RULES = [
    ("A1", "Anchor-view mean", "Mean of the accepted beats in the anchor view only"),
    ("A2", "Mean across views", "Average of the view means of all views"),
    ("A3", "Largest view mean", "Highest view mean, without review"),
    ("A4", "Largest view mean after review",
     "Highest view mean after any view exceeding the anchor by ≥3 mm is reviewed and, if judged artefactual, "
     "excluded"),
]
E1B_WORST = dict(K=3, beat_cv=0.15, view_over=True, window=0.0, S_median_mm=10.0, estimand="T1", axis="AP")
E3_BASE = dict(u="base", axis="AP", cutoff_mm=13.0)
MAIN_T1_WORDS_MAX = 90


def words(text):
    """Word count as a word processor counts it: whitespace-separated tokens that contain a letter or digit."""
    return sum(1 for w in text.split() if any(ch.isalnum() for ch in w))


def table1_main():
    """Main Table 1 and a machine-readable record of every cell for the verification step."""
    src_h, src_w, src_e3 = FULL / "E1E3_e1_headline.csv", AM2 / "E1bE3b_e1_worstcase.csv", FULL / "E1E3_e3_base.csv"
    h = sel(pd.read_csv(src_h), **E1_BASE, u="base", axis="AP", estimand="T1")
    w = sel(pd.read_csv(src_w), **E1B_WORST)
    e3 = sel(pd.read_csv(src_e3), **E3_BASE)
    rows, cells = [], []
    for code, name, what in MAIN_RULES:
        rh, rw, re3 = one(h, estimator=code), one(w, estimator=code), one(e3, estimator=code)
        bias = ci_lohi(rh.bias, rh.bias_lo95, rh.bias_hi95, 2)
        rng = f"{f(rw.bias_min, 2)} to {f(rw.bias_max, 2)}"
        rmse = f(rh.rmse, 2)
        ss = f"{f(100 * re3.sensitivity, 1)} / {f(100 * re3.specificity, 1)}"
        rows.append([name, what, bias, rng, rmse, ss])
        cells += [(code, "bias", rel(src_h), rh.bias, 2), (code, "bias_lo95", rel(src_h), rh.bias_lo95, 2),
                  (code, "bias_hi95", rel(src_h), rh.bias_hi95, 2), (code, "bias_min", rel(src_w), rw.bias_min, 2),
                  (code, "bias_max", rel(src_w), rw.bias_max, 2), (code, "rmse", rel(src_h), rh.rmse, 2),
                  (code, "sensitivity x100", rel(src_e3), 100 * re3.sensitivity, 1),
                  (code, "specificity x100", rel(src_e3), 100 * re3.specificity, 1)]
    hdr = ["Rule", "What is reported", "Bias, base case (mm, 95% MCI)",
           "Bias range, 12 scenarios (mm)", "RMSE, base case (mm)",
           "Sens. / spec. at 13 mm (%)"]
    title = ("**Table 1. Four ways of combining views: what the reader reports and how each rule "
             "performed.**")
    foot = ("Base case (pre-specified): three views, three beats per view, ±15% beat-consistency window, 15% true beat-to-beat "
            "variation, long-axis views underestimating by 20% on average. View-accuracy scenarios: anchor "
            "underestimation 2.4% to 20% crossed with long-axis 8% to 40%, no window. Bias: average of measured "
            "minus true maximal anteroposterior span (positive, overestimation). RMSE, root-mean-square error, which "
            "summarizes the size of individual errors and includes bias. MCI, Monte Carlo "
            "interval (simulation precision only). Sens. / spec., sensitivity / specificity for a true span "
            "≥13 mm. 100,000 simulated patients per condition.")
    nfoot = words(foot)
    assert nfoot <= MAIN_T1_WORDS_MAX, f"Table 1 footnote has {nfoot} words (limit {MAIN_T1_WORDS_MAX})"
    prov = ("<!-- Provenance, Table 1 (not part of the table). Generated by " + SCRIPT + ". "
            f"Bias (95% MCI) and RMSE: {rel(src_h)}, rows u = base (mean anchor underestimation 4.8%, mean "
            "long-axis 20%, view 4 24%), view_over True, K 3, beat_cv 0.15, rho 0.3, N_beats 3, axis AP, "
            "estimand T1; window ±15%, sinus rhythm, prospective acquisition (E1 base case); MCI limits are the "
            "stored bias_lo95 and bias_hi95 (estimate ± 1.96 MCSE). "
            f"Bias range: {rel(src_w)} (E1b, amendment 2), rows K 3, beat_cv 0.15, view_over True, window 0 "
            "(none), S_median_mm 10, axis AP, estimand T1; columns bias_min and bias_max over the 12 combinations "
            "of mean anchor underestimation 2.4%, 4.8%, 10%, 20% and mean long-axis underestimation 8%, 20%, 40%. "
            f"Sensitivity and specificity: {rel(src_e3)} (E3), rows u base, axis AP, cutoff_mm 13 (event: true "
            "AP span ≥13 mm; test positive: reported value ≥13 mm), same base case as E1. A4 settings: warning "
            "3 mm, adjudication 5 mm, detection of true overestimation 0.8, false rejection 0.1. n = 100,000 "
            f"simulated patients per condition. Footnote length {nfoot} words. -->")
    table_text = "\n\n".join([title, md_table(hdr, rows, ["l", "l", "r", "r", "r", "r"]), foot])
    return table_text + "\n\n" + prov, table_text, cells


# ---------------------------------------------------------------- Supplementary Table S6 (v04)
def tableS6():
    rows = [
        ...
        ["Rule", "Largest view mean after review", "A4",
         "Highest view mean after any view exceeding the anchor by ≥3 mm is reviewed and, if judged "
         "artefactual, excluded"],
        ["Rule", "Median across views", "A5", "Median of the view means"],
        ["Rule", "Index beat", "A6",
         "Single anchor-view beat whose preceding and following RR intervals are most nearly equal"],
        ["Rule", "Offset-corrected mean (benchmark)", "A7",
         "Mean of the view means after dividing each by its true view offset; needs simulated truth, so it "
         "cannot be used clinically"],
        ["Target quantity", "True maximal span", "T1", "True maximal jet span on the axis measured"],
        ["Target quantity", "Anchor-view median span (anchor-view median beat span)", "T2",
         "Median beat span in the anchor view, including that view's under- or overestimation but before "
         "instrument and caliper error"],
        ["Analysis", "Beat-window analysis", "E2", "Pre-specified"],
        ["Analysis", "View-rule analysis", "E1", "Pre-specified"],
        ["Analysis", "View-accuracy extension", "E1b", "Amendment 2"],
        ["Analysis", "Cut-off analysis", "E3 and E3b", "E3 pre-specified; E3b amendment 2"],
        ["Analysis", "Cross-view trigger analysis", "E4", "Pre-specified"],
        ["Analysis", "AI validation analysis", "E5", "Pre-specified"],
        ["Analysis", "Shared-error extension", "E5b", "Amendment 2"],
        ["Analysis", "Reference-set sizing analysis", "E6 and E6b", "E6 pre-specified; E6b amendment 2"],
        ["Symbol", "Share of image error reproduced by AI", "λ",
         "Fraction of the image-level error that the AI model reproduces"],
        ["Symbol", "Drift towards AI draft", "w",
         "Fraction of the way an assisted reader moves towards the AI draft"],
    ]
    hdr = ["Type", "Plain name (main text and figures)", "Code (protocol, code and Supplementary Tables S1 to S5)",
           "Meaning"]
    title = ("**Supplementary Table S6. Plain names used in the main text and figures, and the corresponding "
             "codes.**")
    foot = ("The codes are those of the study protocol, its amendment 2, the analysis code and the "
            "result files. RR, interval between successive R waves.")
...
def main():
    ...
    t1_full, t1_text, _ = table1_main()
    doc = ["# Tables, DUO-Max simulation manuscript v04",
           ...
           "by hand; rerun the script. Citation keys ([@key]) follow manuscript v04. Main text: Table 1. "
           "Supplement: Supplementary Tables S1 to S6 (v03 Tables 1 to 4 and Supplementary Table S1 are S1 to "
           "S5). -->",
           "## Main table", t1_full,
           "## Supplementary tables",
           table1(cfg, cfg2), table2(), table3(), table4(), tableS1(), tableS6()]
    text = "\n\n".join(doc) + "\n"
    for bad in ("–", "—"):
        assert bad not in text, f"forbidden dash {bad!r} in output"
    for bad in ("acceptance window", "septal-lateral", "bRIGHT", "doi:10.1016/j.echo.2023.12.002"):
        assert bad not in text, f"forbidden term {bad!r} in output"
