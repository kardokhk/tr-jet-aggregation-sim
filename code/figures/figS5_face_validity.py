...
def main():
    ...
    axa.text(2.0, 3.82 - 0.35, "Hauptmann 2026, 3.82 mm", ha="center", va="top", fontsize=6)
    ...
    axa.set_ylim(0, 15)
    axa.set_ylabel("Inter-reader 95% LoA width (mm)")
    ...
    axb.text(3.72, 0.855, "Singh\n2026", ha="center", va="top", fontsize=6)
    ...
    axc.set_ylabel("View value minus truth (% of mean truth)")
    ...
    axc.legend(hc, lc, loc="upper right", frameon=False, handletextpad=0.4, borderaxespad=0.2,
               handlelength=2.2)

    # ---------------- (d) beat CV
    axd.axhline(15.5, color="#000000", lw=0.8, ls=(0, (4, 2)))
    axd.text(31.5, 14.6, "Moraldo 2013, 15.5%", ha="right", va="top", fontsize=6)
    ...
    items = [("T1 AP (true span)", BASE_L, "T1 (true\nmaximal span)", "#000000", "D", "#000000"),
             ("anchor view 3-beat mean", BASE_L, "Anchor view\n3-beat mean", COLOR["A1"], MARKER["A1"], COLOR["A1"]),
             ("long-axis view single beat", BASE_L, "Long-axis view\nsingle beat", "#6e6e6e", "v", "white"),
             ("long-axis view 3-beat mean", BASE_L, "Long-axis view\n3-beat mean", "#6e6e6e", "s", "#6e6e6e"),
             ("A4 composite", BASE_L, "A4 composite\nrule", COLOR["A4"], MARKER["A4"], "white"),
             ("long-axis view 3-beat mean", 0.50, "Long-axis view\n3-beat mean, L 0.50", "#6e6e6e", "s", "white"),
             ("A4 composite", 0.50, "A4 composite\nrule, L 0.50", COLOR["A4"], MARKER["A4"], "white")]
    x0 = len(items)
    ...
    axe.set_xticks(range(x0 + 1), [it[2] for it in items] + ["Sugiura 2021\nVC width, n = 44"])
    ...
    for ax in (axa, axb, axc, axd, axe):
        ax.grid(False)
    figstyle.panel_labels([axa, axb, axc, axd, axe], list("abcde"), dx=-0.17, dy=1.01)
