...
WIDTH_MM, HEIGHT_MM = 183.0, 128.0
AXIS = "AP"
...
def main():
    ...
    for k, u, xv, m in panels:
        ...
        ax.set_title(UT[u], fontsize=es.FS_TITLE)
        if m == "bias":
            ax.set_ylim(-1.5, 1.7)
            ax.set_ylabel("Bias vs true maximal span (mm)")
        else:
            ax.set_ylim(1.4, 3.0)
            ax.set_ylabel("RMSE vs true maximal span (mm)")
    for k in "bd":
        axd[k].set_ylabel("")
    ...
    for k, u in (("g", "base"), ("h", "low")):
        ...
        ax.annotate("Largest view mean", (100 * s3.beat_cv.iloc[-1], s3.bias.iloc[-1]), xytext=(-4, 3),
                    textcoords="offset points", ha="right", va="bottom", fontsize=es.FS_MIN, color=COLOR["A3"])
        ...
        ax.set_title(UT[u] + ",\n$f_{rej}$ 0.1", fontsize=es.FS_TITLE)
        ax.set_ylabel("Bias vs true maximal span (mm)" if k == "g" else "")
        if k == "h":
            ax.tick_params(labelleft=False)
    hsd = [Line2D([], [], **kw("A4", ls=SD_LS[s], ms=2.5)) for s in (0.0, 0.5, 0.8, 1.0)]
    # v05 (8 pt): the s_det key no longer fits beside the curves in g, so it sits in a strip under the figure,
    # right-aligned below g and h (the only panels with these lines)
    fig.legend(hsd, [f"After review, $s_{{det}}$ {s:g}" for s in (0.0, 0.5, 0.8, 1.0)], loc="outside lower right",
               ncol=4, frameon=False, fontsize=es.FS_MIN, handlelength=2.6, columnspacing=1.2)
    ...
    figstyle.panel_labels([axd[k] for k in "abcdefgh"], list("abcdefgh"), size=es.FS_LETTER)
